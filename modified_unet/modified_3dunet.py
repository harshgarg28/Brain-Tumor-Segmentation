...
################################################################
def improved_unet_model(IMG_HEIGHT, IMG_WIDTH, IMG_DEPTH, IMG_CHANNELS, num_classes):
    #Build the model
    ...
    #Compile the model
    # model.compile(optimizer=Adam(lr=1e-4), loss='categorical_crossentropy', metrics=[MeanIoU(num_classes=num_classes)])
    
    model.summary()
    return model

#Test if everything is working ok. 
model = improved_unet_model(128, 128, 128, 3, 4)
